...
#
function printHelp() {
  echo -e "Cleans entire node.js project"
	echo -e "Use case:"
  ...
  echo -e "  -n|--name - name of the container to be deleted (i.e. 'node'),"
  echo -e "  -p|--port - a port on which node.js server that will be run inside docker's container is listening (i.e. 8081),"
	echo -e "  --ver - version of 'node' container to be removed (i.e. 'latest'),"
	echo -e "  -f|--force - ignores any error occurred in any stage of this script,"
  echo -e "\ni.e.\n"
  echo -e "bash clean.bash -d './Dockerfile' -f"
  echo -e "bash clean.bash -p 8081 --ver 'latest' -f"
}
...
function cleanDir() {
  ...
  echo "Removing files generated by npm..."
